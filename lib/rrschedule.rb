...
module RRSchedule
  class Schedule
    ...
    def dispatch_games(rounds)
      ...
      s=@class_schedule.group_by{|fs| fs[:gamedate]}.sort
    ...
    def dispatch_game(game)
      ...
      if @class_schedule.size>0
        games_this_date = @class_schedule.select{|v| v[:gamedate] == @cur_date}
      ...
      @class_schedule << {:team_a => game.team_a, :team_b => game.team_b, :gamedate => @cur_date, :ps => @cur_ps, :gt => @cur_gt}
  ...
  class Game
    ...
    def to_s
      team_a.to_s + " " + team_b.to_s
    end

  end

  class Round
    attr_accessor :round, :games, :flight, :gamelist
